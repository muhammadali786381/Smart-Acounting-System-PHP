...
class Main{
 private $con;
  ...
  //get any table with sort
   public function getAllSortRecord($table,$colOrder,$order="DESC"){
		$pre_stmt = $this->con->prepare("SELECT * FROM ".$table." ORDER BY ".$table.".".$colOrder." ".$order);
		$pre_stmt->execute() or die($this->con->error);
		$result = $pre_stmt->get_result();
		$rows = array();
		if ($result->num_rows > 0) {
	...
	public function getSingleRecord($table,$pk,$id){
      ...
      //sum transction
       ...
       public function sumValuesBaseTwoCondtion($table,$sumColumm,$pk_1,$id_1,$pk_2,$id_2)
        ...
        //get
        public function CountDatThreeCondition($table,$col_nam_1,$id_1,$col_nam_2,$id_2,$col_nam_3,$id_3){
            //how to use->CountDatTwoCondition("post_service","client_id",2,"status","Completed");
                //$query = $this->con->query("SELECT COUNT(*)AS rows FROM products WHERE status=0");
		//$row = mysqli_fetch_assoc($query);
          // echo  $sql = "SELECT COUNT(*)FROM ".$table." WHERE ".$col_nam_1."='".$id_1."' AND ".$col_nam_2."='".$id_2."'";
            $pre_stmt= $this->con->prepare("SELECT COUNT(*)FROM ".$table." WHERE ".$col_nam_1."='".$id_1."' AND ".$col_nam_2."='".$id_2."' AND ".$col_nam_3."='".$id_3."'");
            $pre_stmt->execute() or die($this->con->error);
            $result = $pre_stmt->get_result();
            $row=$result->fetch_assoc();
            $res=$row['COUNT(*)'];
        ...
        //get data within data range
        public function getDateRangeRecord($table,$field,$date_from,$date_to,$colOrder,$order="DESC"){
             //how to user-> $res=$test->getDateRangeRecord("vouchers","v_date","2020-08-15","2020-08-15");
              $pre_stmt= $this->con->prepare("SELECT ".$table.".* FROM ".$table." WHERE DATE(".$field.") BETWEEN '".$date_from."' AND '".$date_to."' ORDER BY ".$table.".".$colOrder." ".$order);
              $pre_stmt->execute() or die($this->con->error);
	      $result = $pre_stmt->get_result();
		$rows = array();
		if ($result->num_rows > 0) {
